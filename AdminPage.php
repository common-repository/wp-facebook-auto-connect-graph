...
function jfb_admin_page()
{
    ...
    global $opt_jfb_app_id, $opt_jfb_bt_size, $opt_jfb_api_sec, $opt_jfb_email_to, $opt_jfb_delay_redir, $jfb_homepage;
    global $opt_jfb_ask_perms, $opt_jfb_req_perms, $opt_jfb_hide_button, $opt_jfb_mod_done, $opt_jfb_ask_stream, $opt_jfb_stream_content, $opt_jfb_logout_url, $opt_jfb_logout_button;
    ...
    <?php
      ...
      if( isset($_POST['main_opts_updated']) )
      {
          ...
          update_option( $opt_jfb_app_id, $_POST[$opt_jfb_app_id]);
          update_option( $opt_jfb_app_id, $_POST[$opt_jfb_app_id] );
          update_option( $opt_jfb_api_sec, $_POST[$opt_jfb_api_sec] );
          update_option( $opt_jfb_ask_perms, $_POST[$opt_jfb_ask_perms] );
          update_option( $opt_jfb_logout_button, $_POST[$opt_jfb_logout_button] );
          ...
          update_option( $opt_jfb_logout_url, $_POST[$opt_jfb_logout_url] );
          update_option( $opt_jfb_bt_size, $_POST[$opt_jfb_bt_size] );
    ...
    <form action="https://www.paypal.com/cgi-bin/webscr" method="post">
        ...
        <input type="hidden" name="hosted_button_id" value="UHH88A8XQNWW2" />
    ...
    <form name="formMainOptions" method="post" action="">
        <b>Facebook:</b><br />
        <input type="text" size="40" name="<?php echo $opt_jfb_app_id?>" value="<?php echo get_option($opt_jfb_app_id) ?>" /> Application ID<br />
        <input type="text" size="40" name="<?php echo $opt_jfb_api_sec?>" value="<?php echo get_option($opt_jfb_api_sec) ?>" /> API Secret<br /><br />
        <b>Facebook button:</b><br />
        <select name="<?php echo $opt_jfb_bt_size;?>" id="<?php echo $opt_jfb_bt_size;?>" style="width:12em">
            <?php $args = array ('small','medium','large') ?>
            <?php foreach ($args as $sizeToShow) {?>
                <option value="<?php echo $sizeToShow; ?>" <?php if ($sizeToShow == get_option($opt_jfb_bt_size)) {echo 'selected="selected"';} ?>><?php echo $sizeToShow; ?></option>
            <?php } ?>
        </select> Login button<br />
        <input type="checkbox" name="<?php echo $opt_jfb_logout_button?>" value="1" <?php echo get_option($opt_jfb_logout_button)?'checked="checked"':''?> /> Show logout button<br />
        <br /><b>E-Mail:</b><br />
        ...
        <br /><b>Logout url:</b><br />
        <input type="text" size="78" name="<?php echo $opt_jfb_logout_url?>" value="<?php echo get_option($opt_jfb_logout_url) ?>" /> Custom logout url<br />
...
{
    global $jfb_name, $jfb_version, $opt_jfb_valid, $opt_jfb_app_id;
    $msg = get_option($opt_jfb_valid)?"VALID":(!get_option($opt_jfb_app_id)||get_option($opt_jfb_app_id)==''?"NOKEY":"INVALIDKEY");
